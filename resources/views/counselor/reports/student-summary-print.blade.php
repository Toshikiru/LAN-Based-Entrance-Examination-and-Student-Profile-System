...
<body>
    ...
    <p class="meta">{{ $rangeLabel }} &middot; Generated {{ now()->format('M d, Y h:i A') }} &middot; {{ $rows->count() }} student(s)</p>

    <table>
        <thead>
            <tr><th>Student</th><th>School ID</th><th>Exams Taken</th><th>Average %</th><th>Pass Rate</th></tr>
        </thead>
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    <td>{{ $row['name'] }}</td>
                    <td>{{ $row['school_id'] }}</td>
                    <td>{{ $row['exams_taken'] }}</td>
                    <td>{{ $row['average_percentage'] }}%</td>
                    <td class="pass">{{ $row['pass_rate'] }}%</td>
                </tr>
            @empty
                <tr><td colspan="5">No completed exams in this period.</td></tr>
            @endforelse
        </tbody>
    </table>
</body>
